...
> 💖 **Sponsors Only**: 本リポジトリは **GitHub Sponsors** 限定の特典コードです。スポンサーの皆様は商用・非商用問わず自由に利用・カスタマイズ・デプロイいただけます。
...
* 🔐 **親管理者権限 & ガバナンス**: テナントごとのリソース制限（`SAAS_LIMITS`）やサスペンド（一時停止）、アクセスログ監査。
* 🌐 **ハイブリッドルーティング**: デフォルトのパス指定ルーティング（`/w/tenant`）と、独自ドメインによる完全サブドメイン分離（`tenant.domain.com`）の自動切り替え。
* 💳 **Stripe 課金・決済連携 (開発中 / 今後対応予定)**: Stripe Checkout による有料プラン自動決済、Webhook連動、および契約管理ポータル連携。
...
これで、アプリケーションの手前に社内SSO認証ゲートが設置され、許可された社内人間しかアクセスできない二重防御が完了します。
...
## 📄 ライセンス & 利用規約 (License & Commercial Rights)

本リポジトリは **[Sponsor-Only Commercial & SaaS Provider License](./LICENSE)** が適用されます。

* ✅ **許可される利用形態 (Permitted)**:
  * 自社・個人でのデプロイ・カスタマイズ・内部運用。
  * **商用SaaSおよび受託/代行サービスとしての第三者への提供・再販売**（ご自身のCloudflare環境にデプロイし、自社ブランドでクライアントや顧客へSaaSとして有料・無料提供すること）。
* ❌ **禁止事項 (Restricted)**:
  * 本リポジトリの**ソースコードそのもの**を第三者へ共有、公開、再配布、または単体で販売・転売すること。

---

## 💖 スポンサーの皆様へ
cohive の開発および継続的なメンテナンスを支えていただき、心より感謝申し上げます！
